...
All "necessary" favicons are in `src/assets/images/favicon`, and copied over to the root of the output folder. They are regenerated automatically on every Eleventy build, so updating your source SVG and running `npm run build` (or the dev server) keeps the browser tabs in sync without any extra steps.
...
You can place them in that directory manually, or use the script to automate the process (still handy if you need to regenerate icons without running a full build):
